    ...
    protected function LoadData()
    {
        ...
        if ($this->params['review']) {
            ...
            if ($this->params['max_id']) {
                $to_start_link = \CodeIsOk\Application::getUrl('reviews', ['max_id' => 0, 'p' => $p]);
            }
            if (count($snapshots) > $limit) {
                $last = array_pop($snapshots);
                $more_link = \CodeIsOk\Application::getUrl('reviews', ['max_id' => $last['id'], 'p' => $p]);
            }
        }
        ...
        foreach ($snapshots as &$snapshot) {
            ...
            if (\CodeIsOk\Tracker::instance()->enabled()) {
                $ticket_key = \CodeIsOk\Tracker::instance()->parseTicketFromString($ticket_key);
                if (!empty($ticket_key)) {
                    $snapshot['ticket_url'] = \CodeIsOk\Tracker::instance()->getTicketUrl($ticket_key);
                }
            }
