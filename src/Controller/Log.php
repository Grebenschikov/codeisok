...
class Log extends Base
{
    ...
    protected function LoadData()
    {
        ...
        $toHash = null;
        if (!$co) {
            $co = \CodeIsOk\Db::getInstance()->getBranchHead($this->project->GetProject(), $this->params['hash']);
            if ($co) $co = $this->project->GetCommit($co);
            if (!$co) return;
            $toHash = $co->GetHash();
        }
        \CodeIsOk\Log::GetInstance()->Log(__METHOD__, is_object($co) ? get_class($co) : var_export($co, true));
        ...
        if ($revlist) {
            ...
            foreach($revlist_hashes as $revlist_hash) {
                $reviews = \CodeIsOk\Db::getInstance()->findSnapshotsByHash($revlist_hash);
