 ...
 * @subpackage Git
 */
class Tag extends \CodeIsOk\Git\Ref
{
    /**
     * dataRead
     *
    ...
    public static function CompareAge($a, $b)
    {
        $aObj = $a->GetObject();
        $bObj = $b->GetObject();
        if (($aObj instanceof \CodeIsOk\Git\Commit) && ($bObj instanceof \CodeIsOk\Git\Commit)) {
            if ($aObj->GetAge() === $bObj->GetAge()) return 0;
            return ($aObj->GetAge() < $bObj->GetAge() ? -1 : 1);
        }

        if ($aObj instanceof \CodeIsOk\Git\Commit) return 1;

        if ($bObj instanceof \CodeIsOk\Git\Commit) return -1;
