...
abstract class ProjectListBase implements \Iterator
{
    ...
    const GITPHP_SORT_OWNER = 'owner';
    const GITPHP_SORT_AGE = 'age';
     ...
     *
     * @access public
     * @return \CodeIsOk\Git\Project mixed project object or null
    ...
    public function Sort($sortBy = self::GITPHP_SORT_PROJECT)
    {
        switch ($sortBy) {
            case self::GITPHP_SORT_DESCRIPTION:
                uasort($this->projects, array('\CodeIsOk\Git\Project', 'CompareDescription'));
                break;

            case self::GITPHP_SORT_OWNER:
                uasort($this->projects, array('\CodeIsOk\Git\Project', 'CompareOwner'));
                break;

            case self::GITPHP_SORT_AGE:
                uasort($this->projects, array('\CodeIsOk\Git\Project', 'CompareAge'));
                break;

            case self::GITPHP_SORT_PROJECT:
            default:
                uasort($this->projects, array('\CodeIsOk\Git\Project', 'CompareProject'));
                break;
        }
    }
    ...
    {
        if (empty($pattern)) return $this->projects;
    ...
    {
        if (empty($project)) {
            if (isset($projData['project']) && !empty($projData['project'])) $project = $projData['project'];
            else return;
        }
        ...
        if (isset($projData['notify_email']) && is_string($projData['notify_email'])) {
            $projectObj->SetNotifyEmail($projData['notify_email']);
        }
    }
