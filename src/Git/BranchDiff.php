...
class BranchDiff implements \Iterator
{
     ...
     */
    public function __construct($project, $toBranch, $fromBranch, \CodeIsOk\DiffContext $DiffContext)
    {
        ...
        $this->exe = new \CodeIsOk\Git\GitExe($this->project);
    }
    ...
    private function ReadData()
    {
        ...
        foreach ($diffBranchLines as $line) {
            ...
            if ((strlen($trimmed) > 0) && (substr_compare($trimmed, ':', 0, 1) === 0)) {
                try {
                    $fileDiff = new \CodeIsOk\Git\FileDiff($this->project, $trimmed, $this->fromHash, $this->DiffContext, $this->toHash);
