...
class Login extends Base
{
    ...
    protected function ReadQuery()
    {
        ...
        $php_format_cookie = str_replace('.', '_', \CodeIsOk\Jira::getCookieName());
    ...
    protected function LoadData()
    {
        ...
        if ($this->params['post'] && !empty($this->params['login']) && !empty($this->params['password'])) {
            if (\CodeIsOk\Config::GetInstance()->GetAuthMethod() === \CodeIsOk\Config::AUTH_METHOD_CROWD) {
                list ($auth_result, $err) = \CodeIsOk\Jira::instance()->crowdAuthenticatePrincipal(
                    $this->params['login'],
                    $this->params['password']
                );
            } else if (\CodeIsOk\Config::GetInstance()->GetAuthMethod() === \CodeIsOk\Config::AUTH_METHOD_JIRA) {
                list ($auth_result, $err) = \CodeIsOk\Jira::instance()->restAuthenticateByUsernameAndPassword(
                    $this->params['login'],
                    $this->params['password']
                );
            } else if (\CodeIsOk\Config::GetInstance()->GetAuthMethod() === \CodeIsOk\Config::AUTH_METHOD_CONFIG) {
                $auth_user = \CodeIsOk\Config::GetInstance()->GetAuthUserByName($this->params['login']);
            ...
            } else if (\CodeIsOk\Config::GetInstance()->GetAuthMethod() === \CodeIsOk\Config::AUTH_METHOD_REDMINE) {
                list ($auth_result, $err) = \CodeIsOk\Redmine::instance()->restAuthenticateByUsernameAndPassword(
                    $this->params['login'],
                    $this->params['password']
                );
            } else {
            ...
            if (\CodeIsOk\Config::GetInstance()->GetAuthMethod() === \CodeIsOk\Config::AUTH_METHOD_CROWD) {
                list ($auth_result, $err) = \CodeIsOk\Jira::instance()->crowdAuthenticatePrincipalByCookie(
                    $this->params['crowd_token_key']
                );
            } else if (\CodeIsOk\Config::GetInstance()->GetAuthMethod() === \CodeIsOk\Config::AUTH_METHOD_JIRA) {
                list ($auth_result, $err) = \CodeIsOk\Jira::instance()->restAuthenticateByCookie(
                    $this->params['crowd_token_key']
                );
            }
        }
        ...
        $User = null;
        if ($auth_result) {
            $User = \CodeIsOk\User::fromAuthData($auth_result);
            if (\CodeIsOk\Config::GetInstance()->GetValue(\CodeIsOk\Config::CHECK_ACCESS_GROUP)) {
        ...
        if ($User) {
            ...
            if (!empty($this->params['remember'])) {
                ...
                $atlassian_auth_methods = [
                    \CodeIsOk\Config::AUTH_METHOD_CROWD,
                    \CodeIsOk\Config::AUTH_METHOD_JIRA,
                ];
                if (in_array(\CodeIsOk\Config::GetInstance()->GetAuthMethod(), $atlassian_auth_methods)) {
                    setcookie(\CodeIsOk\Jira::getCookieName(), $User->getToken(), $expire, '/', $domain, false, true);
                }
            }
