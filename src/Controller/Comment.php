...
class Comment extends Base
{
    ...
    public function Render()
    {
        ...
        $this->setResponse('log', \CodeIsOk\Log::GetInstance()->getForJson());
    ...
    public function getReviewAction()
    {
        ...
        if ($key = \CodeIsOk\Tracker::instance()->parseTicketFromString($url)) {
            $ticket = \CodeIsOk\Tracker::instance()->getReviewTicketPrefix() . $key;
        }
        $commit_message = isset($_POST['commit_message']) ? $_POST['commit_message'] : '';
        if ($key = \CodeIsOk\Tracker::instance()->parseTicketFromString($commit_message)) {
            $ticket = \CodeIsOk\Tracker::instance()->getReviewTicketPrefix() . $key;
        }
        $hash = $_POST['hash'];
