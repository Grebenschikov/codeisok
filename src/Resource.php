...
class Resource
{
	...
	{
		return \CodeIsOk\Resource::LocaleToName(self::$currentLocale);
	}
	...
	public static function SupportedLocales()
	{
		...
		$locales['en_US'] = \CodeIsOk\Resource::LocaleToName('en_US');
		...
		if ($dh = opendir(GITPHP_LOCALEDIR)) {
			while (($file = readdir($dh)) !== false) {
				...
				if ((strpos($file, '.') !== 0) && is_dir($fullPath) && is_file($fullPath . '/gitphp.mo')) {
					...
					$locales[$file] = \CodeIsOk\Resource::LocaleToName($file);
				}
			}
		}
	...
	public static function FindPreferredLocale($httpAcceptLang)
	{
		...
		$supportedLocales = \CodeIsOk\Resource::SupportedLocales();
