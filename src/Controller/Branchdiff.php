...
class Branchdiff extends DiffBase
{
    ...
    protected function ReadQuery()
    {
        ...
        $this->params['base'] = $this->Session->get($this->project->GetProject() . \CodeIsOk\Session::SESSION_BASE_BRANCH . $this->params['branch'], '');

        if (isset($_REQUEST['base'])) {
            $this->params['base'] = $_REQUEST['base'];
            if (empty($this->params['review'])) {
                $this->Session->set($this->project->GetProject() . \CodeIsOk\Session::SESSION_BASE_BRANCH . $this->params['branch'], $this->params['base']);
            }
        } else if (empty($this->params['base'])) {
            $force_base_per_repo = \CodeIsOk\Config::GetInstance()->GetValue(\CodeIsOk\Config::FORCE_BASE_BRANCH_PER_REPOSITORY, []);
    ...
    private function getReviewNumber()
    {
        ...
        if(!$review_id || empty(\CodeIsOk\Db::getInstance()->getReviewList([$review_id]))){
            return 0;
        }
        return $review_id;
    }
    ...
    protected function LoadData()
    {
        ...
        $toHash = null;
        if (!$co) {
            $co = \CodeIsOk\Db::getInstance()->getBranchHead($this->project->GetProject(), $this->params['branch']);
        ...
        if (in_array($this->project->GetCategory(), \CodeIsOk\Config::GetInstance()->GetValue(\CodeIsOk\Config::SKIP_SUPPRESS_FOR_CATEGORY, []))) {
            $DiffContext->setSkipSuppress(true);
        }
        $branchdiff = new \CodeIsOk\Git\BranchDiff($this->project, $this->params['branch'], $this->params['base'], $DiffContext);
