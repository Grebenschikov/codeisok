...
class JiraRestClient
{
    ...
    public static function getInstance()
    {
        if (empty(self::$instance)) {
            ...
            self::$instance->jira_url = \CodeIsOk\Config::GetInstance()->GetJiraUrl();
            self::$instance->jira_user = \CodeIsOk\Config::GetInstance()->GetJiraUser();
            self::$instance->jira_password = \CodeIsOk\Config::GetInstance()->GetJiraPassword();
        }
    ...
    private function _request($method, $command, $arguments = [])
    {
        ...
        if (\CodeIsOk\Config::GetInstance()->GetAuthMethod() == \CodeIsOk\Config::AUTH_METHOD_JIRA) {
            ...
            if (!empty($User) && !empty($User->getToken())) {
                $header_options['Cookie'] = \CodeIsOk\Jira::REST_COOKIE_NAME . '=' . $User->getToken();
            }
        }
