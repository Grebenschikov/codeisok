...
class Config
{
    ...
    const TRACKER_TYPE_JIRA = \CodeIsOk\Tracker::TRACKER_TYPE_JIRA;
    const TRACKER_TYPE_REDMINE = \CodeIsOk\Tracker::TRACKER_TYPE_REDMINE;
    ...
    public function LoadConfig($configFile)
    {
        if (!is_file($configFile)) {
            throw new \CodeIsOk\MessageException('Could not load config file ' . $configFile, true, 500);
        }

        /** @noinspection PhpIncludeInspection */
        if (($gitphp_conf = include($configFile)) === false) {
            throw new \CodeIsOk\MessageException('Could not read config file ' . $configFile, true, 500);
        }
