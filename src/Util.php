...
class Util
{
    ...
    public static function sendReviewEmail($from, $review_name, $url, $comments, $review_type = 'unified')
    {
        ...
        $Project = \CodeIsOk\Git\ProjectList::GetInstance()->GetProject($first['repo']);
        ...
        foreach ($comments as $comment) {
            ...
            if ($review_type == 'sidebyside') {
                ...
                if (!isset($blob_hashes[$comment['file']][$comment['side']])) {
                    ...
                    $blob_hashes[$comment['file']][$comment['side']] = \CodeIsOk\Git\Blob::getBlobHash($Project, $commit_hash, $comment['file']);
                }
    ...
    protected static function getDiffCached($hash, \CodeIsOk\DiffContext $DiffContext, \CodeIsOk\Git\Project $Project, &$changes_authors)
    {
        ...
        if (!isset($diffs[$hash])) {
            ...
            if (empty($hash_base)) {
                ...
                $diffs[$hash] = new \CodeIsOk\Git\Blob($Project, $hash_head);
            } else {
     ...
     * @param string $file
     * @param \CodeIsOk\Git\BranchDiff|\CodeIsOk\Git\TreeDiff|\CodeIsOk\Git\FileDiff[]|\CodeIsOk\Git\Blob $Diffs
     * @param int $diff_size
     * @return string|array
     */
    public static function insertCommentsToDiffObj($comments, $file, $Diffs, $diff_size = 4)
    {
        $result = [];
        $startLine = 0;
        if ($Diffs instanceof \CodeIsOk\Git\Blob) {
