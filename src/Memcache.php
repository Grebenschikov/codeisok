...
class Memcache
{
	...
	{
		if (!self::$instance) {
			self::$instance = new \CodeIsOk\Memcache();
		}
	...
	public function __construct()
	{
		if (class_exists('Memcached')) {
			...
			$this->memcacheType = \CodeIsOk\Memcache::Memcached;
		} else if (class_exists('Memcache')) {
			$this->memcacheObj = new Memcache();
			$this->memcacheType = \CodeIsOk\Memcache::Memcache;
		} else {
			throw new \CodeIsOk\MessageException(__('The Memcached or Memcache PHP extension is required for Memcache support'), true, 500);
		}
	}
	...
	public function AddServers($servers)
	{
		...
		if ($this->memcacheType == \CodeIsOk\Memcache::Memcached) {
			$this->memcacheObj->addServers($servers);
		} else if ($this->memcacheType == \CodeIsOk\Memcache::Memcache) {
