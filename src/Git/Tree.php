 ...
 * @subpackage Git
 */
class Tree extends \CodeIsOk\Git\FilesystemObject
{
    /**
     * contents
     *
            ...
            if (preg_match("/^([0-9]+) (.+) ([0-9a-fA-F]{40})(\s+[0-9]+|\s+-)?\t(.+)$/", $line, $regs)) {
                switch ($regs[2]) {
                    ...
                    case 'blob':
                        ...
                        $b->SetPath($path);
                        $size = trim($regs[4]);
                        if (!empty($size)) $b->SetSize(\CodeIsOk\Util::humanFilesize($regs[4]));
    ...
    private function ReferenceContents()
    {
        ...
        for ($i = 0; $i < count($this->contents); ++$i) {
            ...
            if ($obj instanceof \CodeIsOk\Git\Tree) {
                $data['type'] = 'tree';
            } else if ($obj instanceof \CodeIsOk\Git\Blob) {
                $data['type'] = 'blob';
