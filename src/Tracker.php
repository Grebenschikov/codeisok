...
class Tracker
{
    ...
    public function __construct()
    {
        ...
        if (\CodeIsOk\Config::GetInstance()->GetUseJiraTracker()) {
            $res = self::TRACKER_TYPE_JIRA;
        } elseif (\CodeIsOk\Config::GetInstance()->GetUseRedmineTracker()) {
    ...
    public function getTicketUrl($ticket_key)
    {
        ...
        switch ($this->tracker_type) {
            case self::TRACKER_TYPE_JIRA:
                $url = \CodeIsOk\Config::GetInstance()->GetJiraUrl() . 'browse/' . $ticket_key;
                break;
            case self::TRACKER_TYPE_REDMINE:
                $url = \CodeIsOk\Redmine::URL . 'issues/' . $ticket_key;
                break;
        }
        return $url;
    }
    ...
    {
        $key = '';
        if (!empty($this->getTicketRegexp()) && preg_match($this->getTicketRegexp(), $string, $m)) {
            $key = $m['ticket'];
        }
        return $key;
    }
    ...
    public function getTicketSummary($ticket_key)
    {
        ...
        switch ($this->tracker_type) {
            case self::TRACKER_TYPE_JIRA:
                if ($Ticket = \CodeIsOk\JiraRestClient::getInstance()->getIssue($ticket_key)) {
                ...
                break;
            case self::TRACKER_TYPE_REDMINE:
                $Issue = \CodeIsOk\RedmineRestClient::getInstance()->getIssue($ticket_key);
    ...
    public function getTicketDeveloperEmail($ticket_key)
    {
        ...
        switch ($this->tracker_type) {
            case self::TRACKER_TYPE_JIRA:
                $Developer = \CodeIsOk\JiraRestClient::getInstance()->getIssueCustomFieldValue($ticket_key, 'Developer');
            ...
            case self::TRACKER_TYPE_REDMINE:
                ...
                if (!empty($Issue->issue) && !empty($Issue->issue->assigned_to) && !empty($Issue->issue->assigned_to->id)) {
                    ...
                    $Developer = \CodeIsOk\RedmineRestClient::getInstance()->getUser($developer_id);
    ...
    public function getUserEmail($user)
    {
        ...
        switch ($this->tracker_type) {
            case self::TRACKER_TYPE_JIRA:
                $JiraUser = \CodeIsOk\JiraRestClient::getInstance()->getUser($user);
                if (!empty($JiraUser) && !empty($JiraUser->emailAddress)) {
                    $res = $JiraUser->emailAddress;
                }
                break;
            case self::TRACKER_TYPE_REDMINE:
                $RedmineUsers = \CodeIsOk\RedmineRestClient::getInstance()->searchUserByName($user);
    ...
    public function addComment($ticket_key, $comment)
    {
        switch ($this->tracker_type) {
            case self::TRACKER_TYPE_JIRA:
                \CodeIsOk\JiraRestClient::getInstance()->addComment($ticket_key, $comment);
                break;
            case self::TRACKER_TYPE_REDMINE:
                \CodeIsOk\RedmineRestClient::getInstance()->addComment($ticket_key, $comment);
                break;
        }
    }
}
