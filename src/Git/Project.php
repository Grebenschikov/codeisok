...
class Project
{
    ...
    public function GetOwner()
    {
        if (empty($this->owner) && !$this->readOwner) {
            $exe = new \CodeIsOk\Git\GitExe($this);
            $args = array();
            $args[] = 'gitweb.owner';
        ...
        while (false !== ($f = readdir($dh))) {
            ...
            if (is_dir($path)) {
                ...
                if (strlen($hash) == 40) {
                    ...
                    try {
                        $this->heads[$path] = new \CodeIsOk\Git\Head($this, $name, $hash);
                    } catch (\Exception $e) {
    ...
    public function ReadRefList()
    {
        ...
        $LogCount = new \CodeIsOk\CountClass(__FUNCTION__);
            ...
            if (preg_match('/^([0-9a-fA-F]{40}) refs\/(tags|heads)\/([^^]+)(\^{})?$/', $line, $regs)) {
                try {
                    ...
                    if ($regs[2] == 'tags') {
                        ...
                        $this->heads[$key] = new \CodeIsOk\Git\Head($this, $regs[3], $regs[1]);
                    }
     ...
     * @return \CodeIsOk\Git\Head head object
     * @throws \Exception
     */
    public function GetHead($head)
    {
        ...
        if (!isset($this->heads[$key])) {
            $this->heads[$key] = new \CodeIsOk\Git\Head($this, $head);
        }
    ...
    public function BatchReadData(array $hashes)
    {
        ...
        $Git = new \CodeIsOk\Git\GitExe($this);
     ...
     *
     * @param array $hash_tags array(hash => \CodeIsOk\Git\Tag)
     */
    ...
    public function GetLog($hash, $count = 50, $skip = 0, $hashBase = null, $revListOptions = [])
    {
        ...
        /** @var $Commit \CodeIsOk\Git\Commit */
    ...
    {
        $exe = new \CodeIsOk\Git\GitExe($this);
        return trim($exe->Execute(GitExe::GIT_DIFF_TREE, ['-r', escapeshellarg($first_tree), escapeshellarg($second_tree), '2>/dev/null']));
    }
    ...
    public function GetObjectHash($abbreviated_hash, $object_type = "")
    {
        ...
        $exe = new \CodeIsOk\Git\GitExe($this);
        $hash = $exe->Execute(GitExe::GIT_REV_PARSE, ['--quiet', '--verify', $search_for, '2>/dev/null']);
        if ($hash) {
            return trim($hash);
        }
        return $hash;
    ...
    private function ReadEpoch()
    {
        ...
        $epochstr = trim($exe->Execute(GitExe::GIT_FOR_EACH_REF, $args));
     ...
     * @return \CodeIsOk\Git\Commit|null
     * @throws \Exception
     */
    public function getMergeBase(string $first_commit, string $second_commit)
    {
        ...
        $hash = trim((new \CodeIsOk\Git\GitExe($this))->Execute(GitExe::GIT_MERGE_BASE, [$first_commit, $second_commit, '2>/dev/null']));
        if (!$hash) {
            return null;
        }
        return new \CodeIsOk\Git\Commit($this, $hash);
    }
    ...
    {
        $args[] = '2>/dev/null';
        $hashes = trim((new \CodeIsOk\Git\GitExe($this))->Execute(GitExe::GIT_REV_LIST, $args));
     ...
     *
     * @see \CodeIsOk\Git\Project::GetUnmergedCommitsCache()
     */
    ...
    public function ForEachRef($args)
    {
        ...
        $args[] = '2>/dev/null';
        $Exec = new \CodeIsOk\Git\GitExe($this);
